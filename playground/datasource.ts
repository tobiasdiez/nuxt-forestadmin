// Taken from: https://raw.githubusercontent.com/ForestAdmin/agent-nodejs/main/packages/_example/src/forest/datasources/typicode.ts
/* eslint-disable */
// @ts-nocheck
...
class TypicodeCollection extends BaseCollection {
  ...
  async list(
    ...
    if (filter.page) {
      request = request.query({
        _start: filter.page.skip,
        _limit: filter.page.limit,
      })
    }
  ...
  private buildRequest(filter: Filter) {
    let request = superagent.get(
      `https://jsonplaceholder.typicode.com/${this.name}s`
    )
...
export function createTypicode(): DataSourceFactory {
...
export default { factory: createTypicode() } satisfies ForestAdminDataSource
